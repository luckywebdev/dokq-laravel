...
<div class="container register">
	<div class="form">
		<form class="form-horizontal" method="post" role="form" action="{{ url('/auth/changepassword') }}">
			<input type="hidden" value="{{$user->authfile}}" name="authfilename" id="authfilename">
			...
			<div class="form-group row {{ $errors->has('username') ? ' has-danger' : '' }}">
				...
				<div class="col-md-2">
					<input type="text" required name="romaname" value="{{ $romaname }}" class="form-control" id="romaname">
				</div>
				<div class="col-md-2">
					<input type="text" required name="numbername" value="{{ $numbername }}" readonly="true" class="form-control" id="numbername">
				</div>
				@if ($errors->has('romaname'))
				<span class="form-control-feedback">
					<span>{{ $errors->first('romaname') }}</span>
				</span>
				@endif
	...
	$(".form-horizontal:first").submit(function(){
		...
		$.get("<?php echo (isset($_SERVER['HTTPS']) ? 'https' : 'http').'://'.$_SERVER['HTTP_HOST'] ?>/user/duppasswordcheck?" + params,
			function(data, status){
				if(data.result=="dup"){
	        		//alert("{{config('consts')['MESSAGES']['PASSWORD_EXIST']}}");
