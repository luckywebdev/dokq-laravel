			...
			@endif
				<div class="col-md-2" style="margin-left: 100px">
					<select class="bs-select form-control">
						<option id="2" value="2"  @if($order == 2) selected @endif>読書日付順</option>
						<option id="0" value="0"  @if($order == 0) selected @endif>著者五十音順</option>
						<option id="1" value="1"  @if($order == 1) selected @endif>タイトル五十音順</option>
					</select>
				</div>
			</div>
							...
							<td class="col-md-1 text-md-center" style="background-color:{{$color}};padding-left:0px;padding-right:0px;">
								<div class="row col-md-12" style="writing-mode:vertical-rl;margin-left:0px;margin-right:0px;padding-left:0px;padding-right:0px;height:300px;">
									<h5 class="font_gogic text-md-left" style="align-self:center;font-family:HGP明朝B;">
										<a class="font_gogic" style="text-decoration:none;font-family:HGP明朝B;"  @if($myBooks[$i*20+(19-$j)]->active >= 3) href="{{url("/book/".$myBooks[$i*20+(19-$j)]->id."/detail")}}" @endif>
										{{$myBooks[$i*20+(19-$j)]->title}}
										</a>
									</h5>
								</div>
								<div class="row col-md-12" style="writing-mode:vertical-rl;margin-left:0px;margin-right:0px;padding-left:0px;padding-right:0px;height:100px;">
									<h5 class="font_gogic text-md-left" style="align-self:center;font-family:HGP明朝B;">
										<a class="font_gogic" style="text-decoration:none;font-family:HGP明朝B;" href="{{url('book/search_books_byauthor?writer_id=' . $myBooks[$i*20+(19-$j)]->writer_id.'&fullname='.$myBooks[$i*20+(19-$j)]->firstname_nick.' '.$myBooks[$i*20+(19-$j)]->lastname_nick)}}">
										{{$myBooks[$i*20+(19-$j)]->firstname_nick.' '.$myBooks[$i*20+(19-$j)]->lastname_nick}}
										</a>
									</h5>
								</div>
							</td>
...
//			});
			...
			$('.bs-select').change(function(){
				...
				$("#order_form").attr("action", "{{url('/mypage/category/'.$id)}}");
				$("#order_form").submit();
			})
		});   
	</script>
