                ...
                <div class="form-group">
                    ...
                    <div class="col-md-4">
                        <select class="form-control input-large select2me" name="booktype_sel" id="booktype_sel" data-placeholder="選択...">
                            <option value=""></option>
                            <option value="data_card_per" <?php if(old('booktype_sel') == 'data_card_per'): ?> selected <?php endif; ?>>個人カード</option>
                            <option value="data_card_org" <?php if(old('booktype_sel') == 'data_card_org'): ?> selected <?php endif; ?>>団体カード</option>
                            <option value="data_card_book" <?php if(old('booktype_sel') == 'data_card_book'): ?> selected <?php endif; ?>>読Q本カード</option>
                            <option value="data_card_quiz" <?php if(old('booktype_sel') == 'data_card_quiz'): ?> selected <?php endif; ?>>読Q本クイズデータ</option>
                            <option value="per_use_history" <?php if(old('booktype_sel') == 'per_use_history'): ?> selected <?php endif; ?>>個人利用履歴</option>
                            <option value="org_use_history" <?php if(old('booktype_sel') == 'org_use_history'): ?> selected <?php endif; ?>>団体利用履歴</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <input class="form-control <?php if(old('booktype_sel')== '' || old('booktype_sel')== 'per_use_history' || old('booktype_sel')== 'org_use_history'): ?> hide <?php endif; ?>" 
                                type="text" id="username" name="username"  value="<?php echo e(old('username')); ?>" 
                                placeholder="<?php if(old('booktype_sel') == 'data_card_book'): ?> 読Q本ID入力 <?php elseif(old('booktype_sel') == 'data_card_quiz'): ?> 読Qクイズ№入力 <?php else: ?> 読Qネーム入力欄 <?php endif; ?>">
                    </div>
                </div>            
                <div class="form-group">
    ...
    <script type="text/javascript">
            ...
            switch(parseInt($(this).val())){
                case 1:
                    <?php foreach (config('consts')['HISTORY']['QUIZTEST_HISTORY'][0]['WORK'] as $i => $work ): ?>
                        kind.push("<?php echo e($work); ?>");
                        html += "<option value='"+"<?php echo e($i); ?>"+"'>"+"<?php echo e($work); ?>"+"</option>";
                    <?php endforeach ?>;
                    break;
                case 2:
        ...
        $("#next_btn").click(function(){
            if($("select[name=booktype_sel]").val() == null || $("select[name=booktype_sel]").val() == ''){
                 $("#alert_text").html("<?php echo e(config('consts')['MESSAGES']['DATATYPE_REQUIRED']); ?>");
                 $("#alertModal").modal();
                 return;
            }
            if($("select[name=period_sel]").val() == ''){
                    $("#alert_text").html("<?php echo e(config('consts')['MESSAGES']['PERIOD_SELECTED']); ?>");
                    $("#alertModal").modal();
                    return;
            }
            if($("select[name=booktype_sel]").val() == 'per_use_history'){
                if($("select[name=period_sel]").val() == ''){
                    ...
                    if($("select[name=per_usehistory_sel]").val() == ''){
                        $("#alert_text").html("<?php echo e(config('consts')['MESSAGES']['USEHISTORY_SELECTED']); ?>");
                        $("#alertModal").modal();
                        return;
                    }
                }
        ...
        $("#display_btn").click(function(){
            ...
            if(!$("#username").hasClass('hide') && $("#username").val() == ''){
                if($("select[name=booktype_sel]").val() == 'data_card_per' || $("select[name=booktype_sel]").val() == 'data_card_org')
                    ...
                    $("#alert_text").html("<?php echo e(config('consts')['MESSAGES']['BOOKID_REQUIRED']); ?>");
                else if($("select[name=booktype_sel]").val() == 'data_card_quiz')
                    $("#alert_text").html("<?php echo e(config('consts')['MESSAGES']['QUIZID_REQUIRED']); ?>");
            ...
            if($("select[name=booktype_sel]").val() == 'data_card_per'){
                ...
                $("#data_result").attr('action', '<?php echo e(url("/admin/bookdata_view")); ?>');
                $("#data_result").submit();
                return;
            }else if($("select[name=booktype_sel]").val() == 'data_card_quiz'){
                $("#data_result").attr('action', '/admin/quizdata_view');
